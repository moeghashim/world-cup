...
| Analytics | Google Analytics GA4, PostHog, `gtag.js` | Loads the GA4 tag with measurement ID `G-RFPJRPKYQR` from the static homepage head; `/posthog` defines the product analytics dashboard contract for the new Projects.dev PostHog resource `worldcup2026-analytics`, the only active PostHog analytics resource in the default environment. PostHog SDK capture is still pending tracking and privacy approval. |
